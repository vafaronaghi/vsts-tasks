...
function setVariable(key:string, value:string):void {
    var envVarName = key.replace('.', '_').toUpperCase();
    process.env[envVarName] = value;
}
...
function setupDefaultMavenTaskRunner():trm.TaskRunner {
    var taskRunner = new trm.TaskRunner('Maven');
    // default required settings
    taskRunner.setInput('mavenVersionSelection', 'Path');
    taskRunner.setInput('mavenPath', '/home/bin/maven2') // Make that checkPath returns true for this filename in the response file
...
// Sets up all of the required default settings in the immediate environment
// This is for testing that does not involve running the task
function setupDefaultInputs():void {
    // default required settings
    setInput('mavenVersionSelection', 'Default');
    setInput('goals', 'package');
    setInput('javaHomeSelection', 'JDKVersion');
    setInput('jdkVersion', 'default');
    setInput('jdkArchitecture', 'x86');
    setInput('testResultsFiles', '**/TEST-*.xml');
    setInput('sqAnalysisEnabled', 'false');
    setInput('mavenPOMFile', 'pom.xml');
}
...
describe('Maven Suite', function() {
    ...
    // it('mavenpmd: Correct maven goals are applied', (done) => {
    //     // Arrange
    ...
    //     // Assert
    //     assert(mvnRun.args.length == 2, 'expecting only one additional argument');
    //     assert(mvnRun.args.indexOf('testArg') > -1, 'should have the test argument');
    //     assert(mvnRun.args.indexOf('pmd:pmd') > -1, 'should have the PMD goal');
    //     done();
    // });

    it('mavenpmd: Correct parsing of PMD XML file', (done) => {
        // Arrange
    ...
    it('mavenpmd: Should throw on malformed XML', (done) => {
        // Arrange
    ...
    it('Maven / PMD: Executes PMD and uploads results when there is only a root module', (done) => {
        // In the test data:
        // /: pom.xml, target/.
        ...
        responseJsonContent.getVariable['build.sourcesDirectory'] = agentSrcDir;
        responseJsonContent.getVariable['build.artifactStagingDirectory'] = agentStgDir;
        ...
        taskRunner.run()
            .then(() => {
                ...
                assert(taskRunner.ran('/home/bin/maven2 -f pom.xml package pmd:pmd'),
    ...
    it('Maven / PMD: Executes PMD and uploads results when multiple modules are present', (done) => {
        // In the test data:
        // /: pom.xml, no target/
        // /util/: pom.xml, target/
    ...
    it('Maven / PMD: Skips PMD goals if PMD is not enabled', (done) => {
        ...
        taskRunner.run()
            .then(() => {
                ...
                assert(taskRunner.ran('/home/bin/maven2 -f pom.xml package'),
