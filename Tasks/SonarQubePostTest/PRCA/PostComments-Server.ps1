...
function InternalInit
{
    ...
    {
        throw "Cannot determine the repository id. Please ensure the agent version is at least 2.103.1"
    }   
    ...
    $request = InvokeByReflection $script:gitClient "GetPullRequestAsync" @([String], [Guid], [int]) @($script:project.ToString(),  [Guid]::Parse($repositoryIdGuid), $pullRequestId)
